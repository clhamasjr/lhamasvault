...
import{Shield,Key,Building2,Users,Eye,EyeOff,Search,Plus,Edit3,Trash2,ChevronDown,ChevronRight,LogOut,LayoutDashboard,FileText,AlertTriangle,CheckCircle,Copy,ExternalLink,Filter,RefreshCw,Clock,Lock,Bot,Store,UserCheck,X,Check,ChevronLeft,Save,Loader2,AlertCircle,Award,Briefcase,UserCog,Network,Monitor}from"lucide-react";
...
function BankCredsPage({showToast}){
  ...
  const grouped=useMemo(()=>{const g={};filtered.forEach(c=>{const bk=c.bank_short_name||c.bank_name;if(!g[bk])g[bk]={secs:{},bank_id:c.bank_id};const sc=c.section_code||"PRINCIPAL";if(!g[bk].secs[sc])g[bk].secs[sc]=[];g[bk].secs[sc].push(c);});return Object.fromEntries(Object.entries(g).sort(([a],[b])=>a.localeCompare(b)));},[filtered]);
  const bankPromoMap=useMemo(()=>{const m={};banks.forEach(b=>{m[b.id]=b.default_promotora||null;});return m;},[banks]);
  ...
  return<div>
    ...
    {Object.entries(grouped).map(([bankName,info])=>{const promo=bankPromoMap[info.bank_id];return(
      <div key={bankName} style={{background:C.card,borderRadius:14,border:`1px solid ${C.bd}`,marginBottom:16,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.bd}`,background:C.bgI,display:"flex",alignItems:"center",gap:10}}>
          <Building2 size={16} color={C.brand}/>
          <span style={{fontSize:15,fontWeight:700,color:C.brand}}>{bankName}</span>
          {promo?<span style={{fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:8,color:"#7C3AED",background:"#F5F3FF"}}>via {promo}</span>:<span style={{fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:8,color:C.ac,background:C.acB}}>DIRETO</span>}
          <span style={{fontSize:12,color:C.txD,background:C.card,padding:"2px 8px",borderRadius:12,marginLeft:"auto"}}>{Object.values(info.secs).flat().length}</span>
        </div>
        {Object.entries(info.secs).map(([secCode,items])=>(
          <div key={secCode}>
            ...
            <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
              <colgroup><col style={{width:"18%"}}/><col style={{width:"10%"}}/><col style={{width:"8%"}}/><col style={{width:"16%"}}/><col style={{width:"10%"}}/><col style={{width:"5%"}}/><col style={{width:"5%"}}/><col style={{width:"6%"}}/><col style={{width:"9%"}}/><col style={{width:"6%"}}/></colgroup>
              <thead><tr>{["Usuário","Senha","Status","Observação","Emprestado","Loja","Robô","Aprov.","Atualiz.",""].map((h,i)=><th key={i} style={{padding:"8px 12px",textAlign:"left",fontSize:10,fontWeight:700,color:C.txD,textTransform:"uppercase",borderBottom:`1px solid ${C.bdL}`}}>{h}</th>)}</tr></thead>
              <tbody>{items.map(c=><tr key={c.id} style={{borderBottom:`1px solid ${C.bdL}`}} onMouseEnter={e=>e.currentTarget.style.background=C.hover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <td style={{padding:"8px 12px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}><code style={{fontFamily:mono,fontSize:11,color:C.txM,background:C.bgI,padding:"2px 6px",borderRadius:4}}>{c.username}</code></td>
                <td style={{padding:"8px 12px"}}><PwCell id={c.id} type="bank" showToast={showToast}/></td>
                <td style={{padding:"8px 12px"}}><Badge status={c.status}/></td>
                <td style={{padding:"8px 12px",color:C.txD,fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.observation||"—"}</td>
                <td style={{padding:"8px 12px"}}>{c.emprestado_para?<span style={{fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:6,color:"#7C3AED",background:"#F5F3FF"}}>{c.emprestado_para}</span>:<span style={{color:C.txD,fontSize:11}}>—</span>}</td>
                <td style={{padding:"8px 12px",fontSize:11,fontWeight:600,color:c.has_store_view?C.sc:C.txD,textAlign:"center"}}>{c.has_store_view?"SIM":"NÃO"}</td>
                <td style={{padding:"8px 12px",fontSize:11,fontWeight:600,color:c.is_robot?C.ac:C.txD,textAlign:"center"}}>{c.is_robot?"SIM":"NÃO"}</td>
                <td style={{padding:"8px 12px",fontSize:11,fontWeight:600,color:c.is_approver?"#DB2777":C.txD,textAlign:"center"}}>{c.is_approver?"SIM":"NÃO"}</td>
                <td style={{padding:"8px 12px",fontSize:11,color:C.txD}}>{c.last_password_update?new Date(c.last_password_update).toLocaleDateString("pt-BR"):"—"}</td>
                <td style={{padding:"8px 12px"}}><div style={{display:"flex",gap:4}}><button onClick={()=>openEdit(c)} style={{background:"none",border:"none",color:C.txD,cursor:"pointer",padding:2,display:"flex"}}><Edit3 size={14}/></button><button onClick={()=>handleDelete(c.id)} style={{background:"none",border:"none",color:C.txD,cursor:"pointer",padding:2,display:"flex"}}><Trash2 size={14}/></button></div></td>
...
function PromCredsPage({showToast}){
  ...
  useEffect(()=>{load();},[]);
  async function load(){setLd(true);const[cr,pr,op]=await Promise.all([supabase.from("promotora_credentials_view").select("*").eq("is_system",false).order("promotora_name").order("person_name"),supabase.from("promotoras").select("*").eq("is_system",false).order("name"),supabase.from("operators").select("*").eq("is_active",true).order("name")]);setCreds(cr.data||[]);setProms(pr.data||[]);setOps(op.data||[]);setLd(false);}
  ...
  return<div>
    ...
    {Object.entries(grouped).map(([pName,items])=>(
      <div key={pName} style={{background:C.card,borderRadius:14,border:`1px solid ${C.bd}`,marginBottom:16,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
          <colgroup><col style={{width:"22%"}}/><col style={{width:"12%"}}/><col style={{width:"14%"}}/><col style={{width:"14%"}}/><col style={{width:"10%"}}/><col style={{width:"9%"}}/><col style={{width:"7%"}}/><col style={{width:"6%"}}/></colgroup>
          <thead><tr>{["Login","Senha","Pessoa","Setor","Cód.Parc","Status","Ativo",""].map((h,i)=><th key={i} style={{padding:"8px 12px",textAlign:"left",fontSize:10,fontWeight:700,color:C.txD,textTransform:"uppercase",borderBottom:`1px solid ${C.bdL}`}}>{h}</th>)}</tr></thead>
          <tbody>{items.map(c=><tr key={c.id} style={{borderBottom:`1px solid ${C.bdL}`}} onMouseEnter={e=>e.currentTarget.style.background=C.hover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            <td style={{padding:"8px 12px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}><code style={{fontFamily:mono,fontSize:11,color:C.txM,background:C.bgI,padding:"2px 6px",borderRadius:4}}>{c.login_username}</code></td>
            <td style={{padding:"8px 12px"}}><PwCell id={c.id} type="promotora" showToast={showToast}/></td>
            <td style={{padding:"8px 12px",color:C.tx,fontWeight:600,fontSize:12}}>{c.person_name||"—"}</td>
            <td style={{padding:"8px 12px"}}>{c.sector?<span style={{fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:6,color:c.sector==="MASTER"?C.brand:c.sector==="FINANCEIRO"?C.wn:c.sector==="CONTRA SENHA"?C.dg:C.txM,background:c.sector==="MASTER"?C.brandB:c.sector==="FINANCEIRO"?C.wnB:c.sector==="CONTRA SENHA"?C.dgB:C.bgI}}>{c.sector}</span>:"—"}</td>
            <td style={{padding:"8px 12px",fontSize:11,color:C.txD,fontFamily:mono,textAlign:"center"}}>{c.partner_code&&c.partner_code!=="---"?c.partner_code:"—"}</td>
            <td style={{padding:"8px 12px"}}><span style={{fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:6,color:C.txM,background:C.bgI}}>{c.status||"ativo"}</span></td>
            <td style={{padding:"8px 12px",fontSize:11,fontWeight:600,color:c.is_active?C.sc:C.dg,textAlign:"center"}}>{c.is_active?"SIM":"NÃO"}</td>
            <td style={{padding:"8px 12px"}}><button onClick={()=>openEdit(c)} style={{background:"none",border:"none",color:C.txD,cursor:"pointer",padding:2,display:"flex"}}><Edit3 size={14}/></button></td>
          </tr>)}</tbody></table></div></div>))}
    <Modal open={!!modal} onClose={()=>setModal(null)} title={modal==="edit"?"Editar credencial":"Nova credencial"} width={600}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
...
// ============================================================
// SENHAS DE SISTEMAS (ClickLev, Gov SP, Backups, etc)
// ============================================================
function SystemsPage({showToast}){
  const[creds,setCreds]=useState([]);const[systems,setSystems]=useState([]);
  const[ld,setLd]=useState(true);const[search,setSearch]=useState("");
  const[modal,setModal]=useState(null);const[saving,setSaving]=useState(false);
  const[form,setForm]=useState({promotora_id:"",login_username:"",password:"",person_name:"",sector:"",is_active:true});
  useEffect(()=>{load();},[]);
  async function load(){setLd(true);const[cr,sy]=await Promise.all([supabase.from("promotora_credentials_view").select("*").eq("is_system",true).order("promotora_name").order("login_username"),supabase.from("promotoras").select("*").eq("is_system",true).order("name")]);setCreds(cr.data||[]);setSystems(sy.data||[]);setLd(false);}
  const filtered=useMemo(()=>{const q=search.toLowerCase();return creds.filter(c=>!q||c.login_username?.toLowerCase().includes(q)||c.person_name?.toLowerCase().includes(q)||c.promotora_name?.toLowerCase().includes(q)||c.sector?.toLowerCase().includes(q));},[creds,search]);
  const grouped=useMemo(()=>{const g={};filtered.forEach(c=>{const p=c.promotora_name||"Outro";if(!g[p])g[p]=[];g[p].push(c);});return Object.fromEntries(Object.entries(g).sort(([a],[b])=>a.localeCompare(b)));},[filtered]);
  function openEdit(c){setModal("edit");setForm({promotora_id:c.promotora_id,login_username:c.login_username,password:"",person_name:c.person_name||"",sector:c.sector||"",is_active:c.is_active,_id:c.id});}
  function openNew(){setModal("new");setForm({promotora_id:"",login_username:"",password:"",person_name:"",sector:"",is_active:true});}
  async function handleSave(){
    if(!form.promotora_id||!form.login_username){showToast("Preencha sistema e login","error");return;}setSaving(true);
    try{const payload={promotora_id:form.promotora_id,login_username:form.login_username,person_name:form.person_name||null,sector:form.sector||null,is_active:form.is_active};
    if(form.password){const{data:enc}=await supabase.rpc("encrypt_password",{plain_text:form.password});payload.password_encrypted=enc;payload.last_password_update=new Date().toISOString();}
    if(modal==="edit"){const{error}=await supabase.from("promotora_credentials").update(payload).eq("id",form._id);if(error)throw error;showToast("Atualizado!","success");}
    else{if(!form.password){showToast("Informe a senha","error");setSaving(false);return;}const{error}=await supabase.from("promotora_credentials").insert(payload);if(error)throw error;showToast("Criado!","success");}
    setModal(null);load();}catch(e){showToast("Erro: "+e.message,"error");}setSaving(false);}
  async function handleDelete(id){if(!confirm("Excluir?"))return;const{error}=await supabase.from("promotora_credentials").delete().eq("id",id);if(error)showToast("Erro","error");else{showToast("Removido","success");load();}}
  if(ld)return<Spinner/>;
  return<div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}><div><h2 style={{fontSize:24,fontWeight:800,color:C.tx}}>Senhas de sistemas</h2><p style={{fontSize:14,color:C.txM,marginTop:4}}>{filtered.length} credenciais</p></div><div style={{display:"flex",gap:8}}><Btn icon={RefreshCw} onClick={load} size="sm">Atualizar</Btn><Btn variant="primary" icon={Plus} onClick={openNew}>Nova</Btn></div></div>
    <div style={{marginBottom:16}}><Inp value={search} onChange={setSearch} placeholder="Buscar login, pessoa, sistema..." icon={Search}/></div>
    {Object.entries(grouped).map(([sName,items])=>(
      <div key={sName} style={{background:C.card,borderRadius:14,border:`1px solid ${C.bd}`,marginBottom:16,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.bd}`,background:C.bgI,display:"flex",alignItems:"center",gap:10}}><Monitor size={16} color="#0891B2"/><span style={{fontSize:15,fontWeight:700,color:"#0891B2"}}>{sName}</span><span style={{fontSize:12,color:C.txD,background:C.card,padding:"2px 8px",borderRadius:12}}>{items.length}</span></div>
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"fixed"}}>
          <colgroup><col style={{width:"25%"}}/><col style={{width:"14%"}}/><col style={{width:"16%"}}/><col style={{width:"16%"}}/><col style={{width:"8%"}}/><col style={{width:"8%"}}/></colgroup>
          <thead><tr>{["Login","Senha","Pessoa","Setor","Ativo",""].map((h,i)=><th key={i} style={{padding:"8px 12px",textAlign:"left",fontSize:10,fontWeight:700,color:C.txD,textTransform:"uppercase",borderBottom:`1px solid ${C.bdL}`}}>{h}</th>)}</tr></thead>
          <tbody>{items.map(c=><tr key={c.id} style={{borderBottom:`1px solid ${C.bdL}`}} onMouseEnter={e=>e.currentTarget.style.background=C.hover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            <td style={{padding:"8px 12px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}><code style={{fontFamily:mono,fontSize:11,color:C.txM,background:C.bgI,padding:"2px 6px",borderRadius:4}}>{c.login_username}</code></td>
            <td style={{padding:"8px 12px"}}><PwCell id={c.id} type="promotora" showToast={showToast}/></td>
            <td style={{padding:"8px 12px",color:C.tx,fontWeight:600,fontSize:12}}>{c.person_name||"—"}</td>
            <td style={{padding:"8px 12px"}}>{c.sector?<span style={{fontSize:10,fontWeight:600,padding:"2px 8px",borderRadius:6,color:"#0891B2",background:"#ECFEFF"}}>{c.sector}</span>:"—"}</td>
            <td style={{padding:"8px 12px",fontSize:11,fontWeight:600,color:c.is_active?C.sc:C.dg,textAlign:"center"}}>{c.is_active?"SIM":"NÃO"}</td>
            <td style={{padding:"8px 12px"}}><div style={{display:"flex",gap:4}}><button onClick={()=>openEdit(c)} style={{background:"none",border:"none",color:C.txD,cursor:"pointer",padding:2,display:"flex"}}><Edit3 size={14}/></button><button onClick={()=>handleDelete(c.id)} style={{background:"none",border:"none",color:C.txD,cursor:"pointer",padding:2,display:"flex"}}><Trash2 size={14}/></button></div></td>
          </tr>)}</tbody></table></div></div>))}
    <Modal open={!!modal} onClose={()=>setModal(null)} title={modal==="edit"?"Editar credencial":"Nova credencial"} width={500}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <Sel label="Sistema" value={form.promotora_id} onChange={v=>setForm({...form,promotora_id:v})} options={systems.map(s=>({value:s.id,label:s.name}))}/>
        <Inp label="Login/Usuário" value={form.login_username} onChange={v=>setForm({...form,login_username:v})} placeholder="793-FRANCINE"/>
        <Inp label={modal==="edit"?"Nova senha (vazio=manter)":"Senha"} value={form.password} onChange={v=>setForm({...form,password:v})} type="password"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <Inp label="Pessoa/Descrição" value={form.person_name} onChange={v=>setForm({...form,person_name:v})} placeholder="FRANCINE, WHATSAPP..."/>
          <Inp label="Setor/Tipo" value={form.sector} onChange={v=>setForm({...form,sector:v})} placeholder="OPERACIONAL, BACKUP..."/>
        </div>
        <Chk label="Ativo" checked={form.is_active} onChange={v=>setForm({...form,is_active:v})} color={C.sc}/>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",paddingTop:16,borderTop:`1px solid ${C.bd}`}}><Btn onClick={()=>setModal(null)}>Cancelar</Btn><Btn variant="primary" icon={Save} onClick={handleSave} loading={saving}>{modal==="edit"?"Salvar":"Criar"}</Btn></div>
      </div>
...
// ============================================================
// USUÁRIOS DO SISTEMA - com aprovação
// ============================================================
function UsersPage({showToast}){
  const[profiles,setProfiles]=useState([]);const[ld,setLd]=useState(true);
  useEffect(()=>{load();},[]);
  async function load(){setLd(true);const{data}=await supabase.from("profiles").select("*").order("is_approved").order("created_at",{ascending:false});setProfiles(data||[]);setLd(false);}
  ...
  async function approve(id){const{error}=await supabase.from("profiles").update({is_approved:true,is_active:true}).eq("id",id);if(error)showToast("Erro","error");else{showToast("Usuário aprovado!","success");load();}}
  async function reject(id){if(!confirm("Rejeitar este usuário?"))return;const{error}=await supabase.from("profiles").delete().eq("id",id);if(error)showToast("Erro","error");else{showToast("Usuário rejeitado","success");load();}}

  async function resetPassword(email){
    try{
      const{error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin});
      if(error)throw error;
      showToast("Link de reset enviado para "+email,"success");
    }catch(e){showToast("Erro: "+e.message,"error");}
  }
  async function setNewPassword(id,email){
    const newPw=prompt(`Nova senha para ${email}:`);
    if(!newPw)return;
    if(newPw.length<6){showToast("Senha deve ter no mínimo 6 caracteres","error");return;}
    try{
      const{error}=await supabase.rpc("admin_reset_password",{target_user_id:id,new_password:newPw});
      if(error)throw error;
      ...
      if(resetErr)showToast("Erro: "+resetErr.message,"error");
      else showToast("Não foi possível alterar direto. Link de reset enviado por email.","warning");
    }
  }
  ...
  if(ld)return<Spinner/>;
  return<div>
    <div style={{marginBottom:20}}><h2 style={{fontSize:24,fontWeight:800,color:C.tx}}>Usuários do sistema</h2><p style={{fontSize:14,color:C.txM,marginTop:4}}>{approved.length} aprovados · {pending.length} pendentes</p></div>

    {pending.length>0&&<div style={{background:C.wnB,borderRadius:14,border:`1px solid ${C.wn}30`,marginBottom:20,overflow:"hidden"}}>
      <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.wn}20`,display:"flex",alignItems:"center",gap:10}}><Clock size={16} color={C.wn}/><span style={{fontSize:14,fontWeight:700,color:C.wn}}>Aguardando aprovação ({pending.length})</span></div>
      {pending.map(p=><div key={p.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 20px",borderBottom:`1px solid ${C.wn}15`}}>
        <div>
          <div style={{fontSize:14,fontWeight:600,color:C.tx}}>{p.full_name}</div>
          <div style={{fontSize:12,color:C.txM,fontFamily:mono}}>{p.email}</div>
          <div style={{fontSize:11,color:C.txD,marginTop:2}}>Solicitou em {new Date(p.created_at).toLocaleString("pt-BR")}</div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <Btn variant="primary" size="sm" icon={Check} onClick={()=>approve(p.id)}>Aprovar</Btn>
          <Btn variant="danger" size="sm" icon={X} onClick={()=>reject(p.id)}>Rejeitar</Btn>
        </div>
      </div>)}
    </div>}

    <div style={{background:C.card,borderRadius:14,border:`1px solid ${C.bd}`,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
      {approved.length===0?<Empty icon={UserCog} title="Nenhum usuário aprovado" desc="Aprove os pendentes acima"/>:
      ...
      <tbody>{approved.map(p=><tr key={p.id} style={{borderBottom:`1px solid ${C.bdL}`}}>
        ...
        <td style={{padding:"10px 16px"}}><div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Btn size="sm" onClick={()=>toggleActive(p.id,p.is_active)}>{p.is_active?"Desativar":"Ativar"}</Btn><Btn size="sm" icon={Lock} onClick={()=>setNewPassword(p.id,p.email)}>Reset senha</Btn></div></td>
      </tr>)}</tbody></table>}
    </div>
  </div>;
}
...
const NAV=[
  ...
  {id:"systems",l:"Sistemas",i:Monitor},
...
function PendingPage(){
  ...
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#F0F4FF 0%,#FEFCE8 100%)"}}>
    <div style={{width:440,background:C.card,borderRadius:24,border:`1px solid ${C.bd}`,padding:"44px 40px",boxShadow:"0 20px 60px rgba(0,0,0,0.08)",textAlign:"center"}}>
      <div style={{width:64,height:64,borderRadius:20,margin:"0 auto 20px",background:C.wnB,display:"flex",alignItems:"center",justifyContent:"center"}}><Clock size={32} color={C.wn}/></div>
      <h2 style={{fontSize:22,fontWeight:800,color:C.tx,margin:"0 0 8px"}}>Aguardando aprovação</h2>
      <p style={{fontSize:14,color:C.txM,margin:"0 0 24px",lineHeight:1.6}}>Olá <strong>{profile?.full_name}</strong>, sua conta foi criada mas ainda precisa ser aprovada por um administrador. Você receberá acesso assim que for liberado.</p>
      <div style={{padding:"12px 16px",borderRadius:12,background:C.bgI,border:`1px solid ${C.bd}`,marginBottom:24}}>
        <div style={{fontSize:12,color:C.txD}}>Email cadastrado</div>
        <div style={{fontSize:14,color:C.tx,fontWeight:600,fontFamily:mono,marginTop:4}}>{profile?.email}</div>
      </div>
      <Btn onClick={signOut} icon={LogOut} style={{width:"100%",justifyContent:"center"}}>Sair</Btn>
    </div>
  </div>;
}
function AppInner(){const{session,loading,profile}=useAuth();if(loading)return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:C.bg}}><Loader2 size={28} color={C.ac} style={{animation:"spin 1s linear infinite"}}/></div>;if(!session)return<LoginPage/>;if(profile&&!profile.is_approved)return<PendingPage/>;return<AppLayout/>;}
