...
function BankCredsPage({showToast}){
  ...
  const[form,setForm]=useState({operator_id:"",bank_id:"",section_id:"",username:"",password:"",status:"amarelo",observation:"",has_store_view:false,is_robot:false,is_approver:false});
  ...
  const filteredSections=useMemo(()=>form.bank_id?sections.filter(s=>s.bank_id===form.bank_id):sections,[sections,form.bank_id]);

  function openEdit(c){setModal("edit");setForm({operator_id:c.operator_id,bank_id:c.bank_id||"",section_id:c.section_id,username:c.username||"",password:"",status:c.status,observation:c.observation||"",has_store_view:c.has_store_view,is_robot:c.is_robot,is_approver:c.is_approver,_id:c.id});}
  function openNew(){setModal("new");setForm({operator_id:"",bank_id:"",section_id:"",username:"",password:"",status:"amarelo",observation:"",has_store_view:false,is_robot:false,is_approver:false});}
    ...
    <Modal open={!!modal} onClose={()=>setModal(null)} title={modal==="edit"?"Editar credencial":"Nova credencial"} width={600}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <Sel label="Seção" value={form.section_id} onChange={v=>setForm({...form,section_id:v})} options={filteredSections.map(s=>({value:s.id,label:(s.banks?.short_name||"")+' - '+s.code}))} placeholder={form.bank_id?"Selecione a seção...":"Selecione o banco primeiro"}/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
...
// ============================================================
// MAPA BANCO×PROMOTORA - Lista global, sem operador
// ============================================================
function MapPage({showToast}){
  const[map,setMap]=useState([]);const[proms,setProms]=useState([]);
  const[ld,setLd]=useState(true);const[search,setSearch]=useState("");
  const[modal,setModal]=useState(null);const[saving,setSaving]=useState(false);
  const[form,setForm]=useState({bank_name:"",promotora_id:"",notes:""});

  useEffect(()=>{load();},[]);
  async function load(){setLd(true);const[mp,pr]=await Promise.all([supabase.from("operator_bank_map").select("*,promotoras(name)").order("bank_name"),supabase.from("promotoras").select("*").order("name")]);setMap(mp.data||[]);setProms(pr.data||[]);setLd(false);}
  const filtered=useMemo(()=>{const q=search.toLowerCase();return map.filter(m=>!q||m.bank_name?.toLowerCase().includes(q)||m.promotora_name?.toLowerCase().includes(q));},[map,search]);

  function openEdit(m){setModal("edit");setForm({bank_name:m.bank_name,promotora_id:m.promotora_id||"",notes:m.notes||"",_id:m.id});}
  function openNew(){setModal("new");setForm({bank_name:"",promotora_id:"",notes:""});}

  async function handleSave(){
    if(!form.bank_name){showToast("Informe o banco","error");return;}
    setSaving(true);
    try{
      const prom=proms.find(p=>p.id===form.promotora_id);
      const payload={bank_name:form.bank_name,promotora_id:form.promotora_id||null,promotora_name:prom?.name||"DIRETO",notes:form.notes||null};
      if(modal==="new")payload.operator_id=map[0]?.operator_id||(await supabase.from("operators").select("id").limit(1).single()).data?.id;
  ...
  return<div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
      <div><h2 style={{fontSize:22,fontWeight:700,color:T.tx,margin:"0 0 6px"}}>Mapa banco × promotora</h2><p style={{fontSize:14,color:T.txM,margin:0}}>{map.length} vínculos</p></div>
    ...
    <div style={{background:T.bgC,borderRadius:14,border:`1px solid ${T.bd}`,overflow:"hidden"}}>
      {filtered.length===0?<Empty icon={Network} title="Nenhum vínculo" desc="Adicione mapeamentos banco→promotora"/>:
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        <thead><tr style={{borderBottom:`1px solid ${T.bd}`}}>{["Banco","Promotora","Notas","Ações"].map((h,i)=><th key={i} style={{padding:"12px 14px",textAlign:"left",fontSize:10,fontWeight:700,color:T.txD,textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
        <tbody>{filtered.map(m=><tr key={m.id} style={{borderBottom:`1px solid ${T.bd}11`}} onMouseEnter={e=>e.currentTarget.style.background=T.bgH} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <td style={{padding:"10px 14px",color:T.tx,fontWeight:500}}>{m.bank_name}</td>
          <td style={{padding:"10px 14px"}}><span style={{fontSize:12,fontWeight:600,padding:"3px 10px",borderRadius:12,color:m.promotora_name&&m.promotora_name!=="DIRETO"?T.ac:T.inf,background:m.promotora_name&&m.promotora_name!=="DIRETO"?T.acB:T.inB}}>{m.promotora_name||"DIRETO"}</span></td>
          <td style={{padding:"10px 14px",color:T.txD,fontSize:12}}>{m.notes||"—"}</td>
          <td style={{padding:"10px 14px"}}><div style={{display:"flex",gap:4}}><button onClick={()=>openEdit(m)} style={{background:"none",border:"none",color:T.txD,cursor:"pointer",padding:4,display:"flex"}}><Edit3 size={14}/></button><button onClick={()=>handleDelete(m.id)} style={{background:"none",border:"none",color:T.txD,cursor:"pointer",padding:4,display:"flex"}}><Trash2 size={14}/></button></div></td>
        </tr>)}</tbody>
      </table>}
    </div>
    <Modal open={!!modal} onClose={()=>setModal(null)} title={modal==="edit"?"Editar vínculo":"Novo vínculo"} width={460}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <Inp label="Banco" value={form.bank_name} onChange={v=>setForm({...form,bank_name:v})} placeholder="AGIBANK, C6 BANK, BRADESCO..."/>
        <Sel label="Promotora" value={form.promotora_id} onChange={v=>setForm({...form,promotora_id:v})} options={[{value:"",label:"DIRETO (sem promotora)"},...proms.map(p=>({value:p.id,label:p.name}))]}/>
